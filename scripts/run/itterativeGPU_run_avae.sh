...
channels=(512)                        # values for --channels
latent_dims=(16)                 # values for --latent_dims
lrs=(0.00025)                          # values for --learning
batch=(256)                                       # values for --batch
epochs=(120)                                      # values for --epochs
gamma=(0.0)    # values for --affinity



# Print the number of jobs to be submitted
echo "Number of jobs to be submitted: $((${#betas[@]} * ${#depths[@]} * ${#channels[@]} * ${#latent_dims[@]} * ${#limits[@]} * ${#batch[@]} * ${#epochs[@]} * ${#lrs[@]} * ${#gamma[@]}))"
                        ...
                        for epoch in "${epochs[@]}"; do
                            for lr in "${lrs[@]}"; do
                                for aff in "${gamma[@]}"; do
                                    ...
                                    job_name="avae_b_${beta}_dep_${depth}_chan_${channel}_lat_${latent_dim}_lim_${limit}_batch_${b}_ep_${epoch}_lr_${lr}_aff_${aff}"
...
echo "Starting affinity-vae-omics script with parameters: beta=${beta}, depth=${depth}, channels=${channel}, latent_dims=${latent_dim}, limit=${limit}, batch=${b}, epochs=${epoch}, lr=${lr}, gamma=${aff}...."

# run the affinity-vae-omics script.
python /iridisfs/ddnb/affinity/affinity-vae-omics/run.py \
    --config_file /iridisfs/ddnb/affinity/experiments/pose2-vae-experiment/config.yml \
